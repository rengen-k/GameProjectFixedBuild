...
public class Level15PlayerController : MonoBehaviour
{
    //-------------------------//
    // Init
    ...
    {
        if (context.ReadValue<Vector2>().x <= -0.5f && (isGrounded || isStableGrounded))
        {
            currentCam = camScript.SwitchState(-1);
            model.transform.Rotate(rotation);
        }
        else if (context.ReadValue<Vector2>().x >= 0.5f && (isGrounded || isStableGrounded))
        {
            currentCam = camScript.SwitchState(1);
            model.transform.Rotate(-rotation);
        }
